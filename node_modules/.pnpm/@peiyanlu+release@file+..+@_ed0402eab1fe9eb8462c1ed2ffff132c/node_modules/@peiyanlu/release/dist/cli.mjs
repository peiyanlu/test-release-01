...
const MSG = {
	INTRO: (pkgName, dryRun) => `🚀  Starting release ${yellow(pkgName)}${n(dryRun)}`,
	OUTRO: (dryRun) => `🎉  Release finished successfully${n(dryRun)}`,
	LOG: {
		SHOW_VERSION: (version) => `🎉  Released ${green(version)}`,
	...
	CHECK: {
		GIT: {
			...
			CHECKED: (name, url) => `Git remote resolved: ${dim(name)} → ${underline(url)}`
		},
		NPM: {
...
const info = (msg$1) => {
	console.log(`${rgb(33, 91, 184)(`ℹ`)} ${gray(msg$1)}`);
};
const success = (msg$1, dryRun) => {
	return `${(dryRun ? rgb(123, 115, 66) : green)("✔")} ${msg$1}`;
};
